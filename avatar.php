    ...
    if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] == 0) { // Si il existe un fichier et qu'il n'y a pas d'erreur'
        ...
        if (!isset($erreur)) { // Vérification si il n'existe pas d'erreur.
            ...
            if (move_uploaded_file($_FILES['avatar']['tmp_name'], 'img/avatar/' . $name)) {
                ...
                bddUpdate('UPDATE membre SET avatar = :avatar WHERE id = :id', [
                    'avatar' => $name, // $name correspond à la génération du nom pour l'image
                    'id' => $_SESSION['id']
                ]); 

                // On créer une condition qui supprime l'ancienne image si elle est différente de celle par défaut.
                if ($membre['avatar'] != 'default-profile.png') {
                    // On supprime un fichier par son nom
                    unlink('img/avatar/' . $membre['avatar']);
                }
                    ...
                    <div class="col-sm-10 col-sm-offset-1">
                        ...
                        <div class="well">
                            ...
                            <div class="text-center">
                                <img src="img/avatar/<?= $membre['avatar'] ?>" alt="avatar" class="img-circle img-fluid" width="50%">
