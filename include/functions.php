...
function accountInformations() :array {
    // Conception d'une variable membre avec la fonction bddSelect associé.
    // On sélectionne tous les champs de la table membre. avec pour clause un id qu'on aura marqué.
    // En second argument on récupère via la superglobale session l'id.
    $membre = bddSelect('SELECT * FROM membre WHERE id = ?', [$_SESSION['id']]);
    // On retourne le $membre don l'indice est 0
    return $membre[0];
}
